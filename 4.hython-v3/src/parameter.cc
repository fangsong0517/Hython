...
void Parameter::def_param(std::string name, SIValue val) {
    if (memory.find(name) != memory.end()) {
        #ifdef DEBUG
        for (auto x : memory) {
            std::cout << "debug log(memory) : " << x.first << " " << x.second->toString() << std::endl;
        }
        #endif
...
void Parameter::set_param(std::string name, SIValue val) {
    if (memory.find(name) == memory.end()) {
        if (this->next) {
...
SIValue Parameter::get_param(std::string name) {
    if (memory.find(name) == memory.end()) {
